...
public:
    ...
    bool charToBool(char c); // Function to convert chars to boolean values 
    void rotateLeft(Block* origBlock, Block* newBlock); // rotates left and set left rotation of the block to it.
    void rotateRight(Block* origBlock, Block* block);  // rotates left and set left rotation of the block to it.
    void printBlock(const Block* block); // prints block
};
