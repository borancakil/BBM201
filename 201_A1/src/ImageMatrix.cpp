...
ImageMatrix& ImageMatrix::operator=(const ImageMatrix &other) {
    if (this != &other) { // Check for self-assignment
        ...
        // Copy the dimensions from the source
        this->height = other.get_height();
        this->width = other.get_width();
...
ImageMatrix ImageMatrix::operator+(const ImageMatrix &other) const {
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j)
                result.set_data(i,j,this->data[i][j] + other.get_data(i, j));
        }
    return result;
}
...
ImageMatrix ImageMatrix::operator*(const double &scalar) const {
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            result.set_data(i,j,this->data[i][j] * scalar);
        }
    }
    return result;
}
...
// Setter function to access the data at the index (i, j)
